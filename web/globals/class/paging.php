...
class paging
{
	...
	function print_link($query_vars,$separator='',$mode='relative')
	{
		...
		if ($this->p["count"]>$this->p["baris"]) {
			...
			// print prev
			if ($this->page>1)
			$print_link .= '<a href="'.$this->page_vars.'1" title="Goto First Page : 1">First</a>'.$separator.'<a href="'.$this->page_vars.($this->page-1).'" title="Previous Page : '.($this->page-1).'">'.$this->prev.'</a>';
			...
			if ($this->page<>1)
			{
				for ($i=$this->p["bawah"];$i<=$this->page-1;$i++)
					$print_link .= $separator.'<a href="'.$this->page_vars.$i.'" title="Page '.number($i,$this->number).'">'.number($i,$this->number).'</a> ';
			}
			// print active
			...
			$print_link .= '<a href="'.$this->page_vars.$i.'" title="Page '.number($i,$this->number).'">'.number($i,$this->number).'</a>'.$separator;

			// print next
			if ($this->page<$this->p["total_page"])
			$print_link .= '<a href="'.$this->page_vars.($this->page+1).'" title="Next Page : '.($this->page+1).'">'.$this->next.'</a>'.$separator.'<a href="'.$this->page_vars.$this->p["total_page"].'" title="Goto Last Page : '.$this->p["total_page"].'">Last</a>';
